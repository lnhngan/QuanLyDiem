...
@section('content')
        ...
        <div class="card-body">
            <form action="{{ route('giaovien.diem.danh-sach') }}" method="GET">
                <div class="row align-items-end">
                    ...
                    @if(request('phan_cong_id'))
                    <div class="col-md-4 mb-3 text-md-end">
                        <a href="{{ route('giaovien.diem.nhap-nhanh', ['phan_cong_id' => request('phan_cong_id')]) }}" class="btn btn-success fw-bold">
                            <i class="bi bi-pencil-square"></i> Vào Nhập Điểm Lớp Này
                        </a>
                    </div>
                    @endif
                </div>
                ...
                <div class="table-responsive">
                    <table class="table table-bordered table-hover text-center align-middle mb-0">
                        ...
                        <tbody>
                            @forelse($hocSinhs as $key => $hs)
                                <tr>
                                    ...
                                    @foreach($loaiDiems as $ld)
                                        ...
                                        <td>
                                            @if($diem)
                                                <span class="{{ $diem->diem_so < 5 ? 'text-danger' : 'text-success' }} fw-bold"
                                                    style="font-size: 1.1rem;">
                                                    {{ number_format($diem->diem_so, 1) }}
                                                </span>
                                                <a href="{{ route('giaovien.diem.sua', $diem->id) }}" class="text-warning ms-2"
                                                    title="Sửa điểm"><i class="bi bi-pencil-fill"></i></a>
                                            @else
                            ...
                            @empty
                                <tr>
                                    <td colspan="{{ 4 + $loaiDiems->count() }}" class="text-center py-4">Lớp này hiện chưa có danh
                                        sách học sinh.</td>
                                </tr>
                            @endforelse
                        </tbody>
    ...
    @else
        <div class="alert alert-info shadow-sm">
            <i class="bi bi-info-circle-fill me-2"></i> Vui lòng chọn một Lớp & Môn học ở hộp thoại phía trên để xem bảng điểm
            chi tiết.
        </div>
    @endif
@endsection